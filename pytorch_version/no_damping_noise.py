...
class PIGP:
    ...
    def __init__(self, X, y, X_ep, device, jitter=1e-6):
        self.device = device
        ...
        self.N = y.size
        self.d = X.shape[1]
        self.X_col = torch.tensor(X_ep, device=self.device, requires_grad=False)
    ...
    def negELBO(self):
        K_hh = self.kernel.matrix(self.X, torch.exp(self.log_ls_h))
        ...
        K = torch.cat((torch.cat((K_hh, K_u_tt.T), 1), torch.cat((K_u_tt, K_u_tt_u_tt), 1)), 0)
        A = torch.cholesky(K)
    ...
    def _callback(self, Xte, yte, epoch):
        with torch.no_grad():
            ...
            if self.best_RMSE is None or self.best_RMSE > err_te:
                ...
                plt.title(ti)
                plt.savefig("no_damping_noise")
                plt.cla()

    def pred(self, Xte, cal_llh=False):
...
def test(M):
    ...
    Y_train = (pd.read_csv(infile1)).to_numpy()[:, -1].reshape([-1, 1]) + np.sqrt(0.1) * np.random.randn(50, 1)
