...
class PIGP:
    ...
    def __init__(self, X, y, X_col, device, jitter=1e-6):
        ...
        self.M = self.N_concat + 2 * self.N_col
    ...
    def negELBO(self):
        ...
        elbo = (-kl + 0.5 * self.N * self.log_tau - 0.5 * torch.exp(self.log_tau) * torch.sum(torch.square(f[0:self.N].view(-1, 1) - self.y)) + 0.5 * self.N_col * self.log_v - 0.5 * torch.exp(self.log_v) * torch.sum(
            torch.square(f[self.N_concat + self.N_col:self.N_concat + 2 * self.N_col].view(-1, 1) - 0.0001 * f[self.N_concat:self.N_concat + self.N_col].view(-1, 1) + 5 *
                         (f[self.N:self.N_concat]**3).view(-1, 1) - 5 * f[self.N:self.N_concat].view(-1, 1))))
        return -elbo
    ...
    def _callback(self, Xte, yte, epoch):
        with torch.no_grad():
            ...
            pred_mean = torch.matmul(K_te, torch.linalg.solve(K, f[0:self.N_concat + 2 * self.N_col]))
            ...
            if self.best_RMSE is None or self.best_RMSE > err_te:
                ...
                plt.title(ti)
                plt.savefig("allen_cahn")
                plt.clf()
                plt.imshow(pred_stds, cmap="hot")
                ti = "RMSE " + str(err_te.cpu().numpy())
                plt.title(ti)
                plt.savefig("allen_cahn_var")
                plt.clf()
