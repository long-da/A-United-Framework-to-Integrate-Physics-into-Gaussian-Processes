...
class KernelARD2:
    ...
    def cross_p(self, Kmn, X1, X2, d, ls):
        s = ls[d]
        D = X1[:, d:d + 1] - X2[:, d:d + 1].T
        res = Kmn * (-4 / s**2 * D**2 + 2 / s)
        return res

    def matrix_p(self, Kmm, X, d, ls):
        K = self.cross_p(Kmm, X, X, d, ls)
        Ijit = self.jitter * torch.eye(X.shape[0]).to(K.device)
        K = K + Ijit
        return K
    ...
    def cross_p_pp_0(self, kmn, X, Xp, ls, d1, d2):
        s1 = ls[d1]
        s2 = ls[d2]
        D1 = X[:, d1:d1 + 1] - Xp[:, d1:d1 + 1].T
        D2 = X[:, d2:d2 + 1] - Xp[:, d2:d2 + 1].T
        res = 4 * (1 - 2 * torch.square(D2) / s2) * (D1) / (s1 * s2) * kmn
        return res
    ...
    def cross_pp(self, Kmn, X, Xp, d, ls):
        s = ls[d]
        D = X[:, d:d + 1] - Xp[:, d:d + 1].T
        res = Kmn * (16 / torch.pow(s, 4) * torch.pow(D, 4) - 48 / torch.pow(s, 3) * torch.pow(D, 2) + 12 / torch.square(s))
        return res
    ...
    def matrix_pp(self, Kmm, X, d, ls):
        K = self.cross_pp(Kmm, X, X, d, ls)
        Ijit = self.jitter * torch.eye(X.shape[0]).to(K.device)
        K = K + Ijit
        return K
