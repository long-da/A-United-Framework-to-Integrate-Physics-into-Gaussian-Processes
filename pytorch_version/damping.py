...
class PIGP:
    ...
    def __init__(self, X, y, X_col, device, jitter=1e-6):
        ...
        self.N_con = self.N + self.N_col
        self.B = torch.tensor(0.0, device=self.device, requires_grad=True)
        self.jitter = jitter
        self.log_ls_h = torch.tensor(np.zeros(self.d), device=self.device, requires_grad=True)
        ...
        self.L1 = torch.tensor(-20 * torch.ones(self.M, 1), device=self.device, requires_grad=True)
    ...
    def negELBO(self):
        ...
        K_u_t_u_t = self.kernel.matrix_p(K_u2, self.X_col, 0, torch.exp(self.log_ls_h))
        K_u_t = self.kernel.cross_p_0(K_u1, self.X_col, self.X, 0, torch.exp(self.log_ls_h))
        K_u_t_u_tt = self.kernel.cross_p_pp(K_u2, self.X_col, self.X_col, 0, torch.exp(self.log_ls_h))
        K = torch.cat(
            (
                torch.cat((K_u_u, K_u_tt.T, K_u_t.T), 1),
                torch.cat((K_u_tt, K_u_tt_u_tt, K_u_t_u_tt.T), 1),
                torch.cat((K_u_t, K_u_t_u_tt, K_u_t_u_t), 1),
            ),
            0,
        )
        A = torch.cholesky(K)
        ...
        elbo = (-kl + 0.5 * self.N * self.log_tau - 0.5 * torch.exp(self.log_tau) * torch.sum(torch.square(f[0:self.N].view(-1, 1) - self.y)) + 0.5 * self.N_col * self.log_v - 0.5 * torch.exp(self.log_v) *
                torch.sum(torch.square(f[self.N_con:self.N_con + self.N_col].view(-1, 1) + torch.sin(f[self.N:self.N_con]).view(-1, 1) + torch.exp(self.B) * f[self.N_con + self.N_col:self.N_con + 2 * self.N_col].view(-1, 1))))
        return -elbo
    ...
    def train(self, Xte, yte, lr, max_epochs=100):
        ...
        self.yte = yte
        paras = [self.mu, self.L1, self.L2, self.log_ls_h, self.log_tau, self.log_v, self.B]
    ...
    def _callback(self, Xte, yte, epoch):
        with torch.no_grad():
            ...
            err_te = torch.sqrt(torch.mean(torch.square(pred_mean - yte.view(-1, 1))))
    ...
    def pred(self, Xte):
        ...
        K_ut_te = self.kernel.cross_p_0(K_u2_te.T, self.X_col, Xte, 0, torch.exp(self.log_ls_h))
        K_te = torch.cat((K_u1_te, K_utt_te.T, K_ut_te.T), 1)
        pred_mean = torch.matmul(K_te, torch.linalg.solve(K, f2[0:self.N_con + 2 * self.N_col]))
        A1 = torch.cholesky(K)
        t1 = torch.linalg.solve(A1.T, Ltril[0:self.N_con + 2 * self.N_col])
        t2 = torch.matmul(t1, t1.T)
        pred_stds = (1 + self.jitter - (K_te * torch.linalg.solve(K, K_te.T).T).sum(1) + (K_te * torch.matmul(t2, K_te.T).T).sum(1))
        pred_stds = pred_stds.view(-1, 1)
...
def test(M):
    ...
    model_PIGP = PIGP(X_train, Y_train, X_col, torch.device("cpu"))
