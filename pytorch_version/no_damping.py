...
class PIGP:
    ...
    def __init__(self, X, y, X_col, device, jitter=1e-6):
        self.device = device
        self.y = torch.tensor(y.reshape([y.size, 1]), device=self.device, requires_grad=False)
        ...
        self.M = self.N_con + self.N_col
        self.mu = torch.tensor(torch.zeros([self.M, 1]), device=self.device, requires_grad=True)
        self.L1 = torch.tensor(5 * torch.ones(self.M, 1), device=self.device, requires_grad=True)
        self.L2 = torch.tensor(5 * torch.ones(self.M, self.M), device=self.device, requires_grad=True)
    ...
    def negELBO(self):
        ...
        K = torch.cat((torch.cat((K_u_u, K_u_tt.T), 1), torch.cat((K_u_tt, K_u_tt_u_tt), 1)), 0)
        A = torch.cholesky(K)
        ...
        elbo = (-kl + 0.5 * self.N * self.log_tau - 0.5 * torch.exp(self.log_tau) * torch.sum(torch.square(f[0:self.N].view(-1, 1) - self.y)) + 0.5 * self.N_col * self.log_v -
                0.5 * torch.exp(self.log_v) * torch.sum(torch.square(f[self.N_con:self.N_con + self.N_col].view(-1, 1) + torch.sin(f[self.N:self.N_con]).view(-1, 1))))
        return -elbo
    ...
    def _callback(self, Xte, yte, epoch):
        with torch.no_grad():
            ...
            if self.best_RMSE is None or self.best_RMSE > err_te:
                ...
                print("Epoch", self.epoch, "mu rmse: ", err_te)
    ...
    def pred(self, Xte):
        ...
        K = torch.cat((torch.cat((K_u_u, K_utt.T), 1), torch.cat((K_utt, K_utt_utt), 1)), 0)
        A = torch.cholesky(K)
        ...
        pred_mean = torch.matmul(K_te, torch.linalg.solve(K, f[0:self.N_con + self.N_col]))
        A1 = torch.cholesky(K)
